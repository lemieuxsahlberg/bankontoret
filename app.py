...
def ensure_profile_exists(user_id, display_name):
    ...
    if not rows:
        supabase.table("profiles").insert({"user_id": user_id, "display_name": wanted_name}).execute()
...
def upsert_round_hole(round_id, hole, total_strokes, notes, shot_rows):
    ...
    if int(total_strokes) == 1:
        ...
        if payload_rows:
            if not payload_rows[-1]["went_in"]:
                payload_rows[-1]["went_in"] = True
            supabase.table("shots").insert(payload_rows).execute()
...
def get_course_hole_averages(user_id, course_id):
    rounds = supabase.table("rounds").select("id").eq("user_id", user_id).eq("course_id", course_id).execute().data or []
    if not rounds:
        return []
    all_round_holes = []
    for rnd in rounds:
        all_round_holes.extend(get_round_holes(rnd["id"]))
    if not all_round_holes:
        return []
    course_holes = get_course_holes(course_id)
    hole_number_map = {h["id"]: h["hole_number"] for h in course_holes}
    rows = [{"Rata": hole_number_map.get(rh["course_hole_id"], rh.get("hole_sequence_number")), "Lyönnit": rh.get("total_strokes", 0)} for rh in all_round_holes]
...
def get_analysis_metrics(user_id):
    ...
    metrics = {
        "piikki": round(pd.Series([1 if h.get("went_straight_in") else 0 for h in all_round_holes]).mean() * 100, 1),
    ...
    if eterniitti_scores:
        ...
        metrics["eterniitti"] = {
            ...
            "pelastettu_kakkoseen_pct": round((pelastettu_count / non_piikki_total) * 100, 1) if non_piikki_total > 0 else None,
            "jatkuu_huono_pct": round((jatkuu_count / non_piikki_total) * 100, 1) if non_piikki_total > 0 else None,
        }
...
def render_new_round(user_id):
    ...
    if st.session_state.current_round_id is None:
        with st.form("start_round"):
            ...
            if st.form_submit_button("Aloita kierros", disabled=not can_start):
                try:
                    r = supabase.table("rounds").insert({"user_id": user_id, "course_id": course["id"], "played_at": played_at.isoformat(), "visibility": visibility, "notes": notes.strip() or None, "round_type": round_type}).execute()
    ...
    strokes = st.radio("Lyönnit", list(range(1, 8)), horizontal=True, index=list(range(1,8)).index(default_strokes if default_strokes in list(range(1,8)) else 1), key=f"strokes_{hole_id}_{pos}")
...
def render_history(user_id):
    ...
    for rnd in rounds:
        ...
        rows.append({"round_id": rnd["id"], "Päivä": rnd.get("played_at"), "Kenttä": course.get("name", "Tuntematon"), "Tyyppi": rnd.get("round_type", "harjoitus"), "Alusta": course.get("surface") or "–", "Par": PAR_BY_SURFACE.get(course.get("surface"), "–"), "Ratoja": len(rhs), "Tulos": total, "Muistiinpanot": rnd.get("notes") or "", "color": color})

    st.markdown("### Kierrokset")
    header = "<table style='width:100%; border-collapse:collapse'><tr><th align='left'>Päivä</th><th align='left'>Kenttä</th><th align='left'>Tyyppi</th><th align='left'>Alusta</th><th align='left'>Par</th><th align='left'>Ratoja</th><th align='left'>Tulos</th><th align='left'>Muistiinpanot</th></tr>"
    body = ""
    for row in rows:
        body += f"<tr><td>{row['Päivä']}</td><td>{row['Kenttä']}</td><td>{row['Tyyppi']}</td><td>{row['Alusta']}</td><td>{row['Par']}</td><td>{row['Ratoja']}</td><td style='color:{row['color']}; font-weight:700'>{row['Tulos']}</td><td>{row['Muistiinpanot']}</td></tr>"
    ...
    avg_practice = round(df[df["Tyyppi"] == "harjoitus"]["Tulos"].mean(), 2) if not df[df["Tyyppi"] == "harjoitus"].empty else None
    avg_comp = round(df[df["Tyyppi"] == "kisa"]["Tulos"].mean(), 2) if not df[df["Tyyppi"] == "kisa"].empty else None
    c1, c2, c3 = st.columns(3)
    c1.metric("Kaikkien kierrosten keskiarvo", avg_all if avg_all is not None else "–")
    c2.metric("Harjoituskierrosten keskiarvo", avg_practice if avg_practice is not None else "–")
    ...
    with st.form("edit_round_form"):
        ...
        if save_changes:
            try:
                supabase.table("rounds").update({"round_type": round_type, "notes": notes.strip() or None}).eq("id", selected_round_id).execute()
                for rh in rhs:
