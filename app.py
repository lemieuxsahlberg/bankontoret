...
def upsert_round_hole(round_id, hole, total_strokes, notes, shot_rows):
    existing = supabase.table("round_holes").select("*").eq("round_id", round_id).eq("hole_sequence_number", hole["hole_number"]).execute().data or []
    payload = {
        "round_id": round_id,
...
def get_course_hole_averages(user_id, course_id):
    ...
    for rh in round_holes_all:
        rows.append({
            "Rata": hole_number_map.get(rh["course_hole_id"], rh.get("hole_sequence_number")),
...
def simple_metrics(user_id):
    ...
    piikki = round(pd.Series([1 if h.get("went_straight_in") else 0 for h in all_holes]).mean() * 100, 1)
        ...
        for i in range(1, int(strokes) + 1):
            ...
            with c5:
                speed_error = st.radio(
                    ...
                    format_func=lambda x: {"none": "ei", "too_slow": "hidas", "too_hard": "liian luja"}[x],
                )
            shot_rows.append({
                "shot_number": i,
...
def render_history(user_id):
    ...
    display_df = df[["Päivä", "Kenttä", "Tyyppi", "Alusta", "Par", "Ratoja", "Tulos", "Muistiinpanot"]].copy()

    def style_scores(row):
        color = df.loc[row.name, "_score_color"]
        return ['color: ' + color + '; font-weight: 700' if col == "Tulos" else '' for col in display_df.columns]

    st.dataframe(display_df.style.apply(style_scores, axis=1), use_container_width=True, hide_index=True)
    ...
    round_map = {f"{row['Päivä']} – {row['Kenttä']} ({row['Tulos']})": row["round_id"] for _, row in df.iterrows()}
...
def render_analysis(user_id):
    ...
    metrics = simple_metrics(user_id)
    if not metrics:
